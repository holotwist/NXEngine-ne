...
public:
  ...
  void playSfx(SFX snd, int32_t loop = 0);
  void playSfxResampled(SFX snd, uint32_t percent);
  void stopSfx(SFX snd);
  ...
  uint32_t currentSong() const { return _currentSong; }
  uint32_t lastSong() const { return _lastSong; }
  void fadeMusic();
  void runFade();
  void pause();
  void resume();
  ...
  std::vector<std::string> &music_dir_names() { return _music_dir_names; }

  // Callback internal for miniaudio
  void audioCallback(void *pOutput, const void *pInput, ma_uint32 frameCount);
